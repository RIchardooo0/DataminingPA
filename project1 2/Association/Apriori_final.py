...
#########################
###Author: Yin gong  Junsong Huang Yi Jin
            ...
            for son in parts[2]:
                for line in range(len(df.RULE)):
                    ...
                    if set([son]).issubset(set(father_candidate)):
                        ...
                        print(set([son]))
                        print("father")
                        print(set(father_candidate))
                        counter[line] += 1
            for index in range(len(counter)):
                if counter[index] == 1:
                    counter_set.add(index)
            total_set = counter_set
            print("total_set = "+str(total_set))
        data = df.RULE
    elif parts[0] == "HEAD":
        if parts[1] == "ANY":
            for son in parts[2]:
...
def queryTemp3(template3, df):
    ...
    if parts[0] == "1or1":
        ...
        count1_set,count1,data1 = queryTemp1(str(parts[1:4]), df)
        count2_set,count2,data2 = queryTemp2(str(parts[4:]), df)
        print(str(count1_set))
        print(str(count2_set))
        count_set = count1_set.intersection(count2_set)
                ...
                if i==1: # if the candidate item-length is 1
                    ...
                    for item in all_frequentSet[h]:
                        ...
                        if conf>=int(confidence)/100:
                            ...
                            char2 = ','
                            Chart.loc[len(Chart)] = pd.Series({'RULE':char2.join(Set_temp)+','+item,'HEAD':char2.join(Set_temp),'BODY':str(item),'CONFIDENCE':conf})
...
#######################################################################################
                                ...
                                if conf >= int(confidence)/100:
                                    ...
                                    i_length_set.append(list_after_sort)
                                    char1 = ','
                                    print(char1.join(list_after_sort))

                                    Chart.loc[len(Chart)] = pd.Series(
                                        {'RULE': char1.join(Set_temp)+ ',' + char1.join(list_after_sort),
                                         'HEAD': char1.join(Set_temp), 'BODY': char1.join(list_after_sort),
                                         'CONFIDENCE': conf})
                    # print("num_"+str(i)+"is "+str(i_length_set))

                    previous = i_length_set
    print("number of rules is "+str(counter))
